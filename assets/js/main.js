...
-------------------------------------------------------------*/
 ...
 --------------------------------------------------------------*/
  ...
  var testiSliderSwiper = new Swiper(".home1-testi-slider1", {
    speed: 1000,
    loop: true,               // Ensures the right side stays filled
    centeredSlides: true,     // Keeps active card in the middle
    roundLengths: true,       // Fixes blurry text/images on some screens
    autoplay: {
      delay: 5000,
      disableOnInteraction: false,
    },
    ...
    breakpoints: {
      ...
      768: {
        slidesPerView: 2,      // 2 items on tablet
        centeredSlides: false, // Prevents big gaps on tablet
        spaceBetween: 20,
      },
      1200: {
        slidesPerView: 3,      // 3 items on desktop
        spaceBetween: 30,
        centeredSlides: true,  // Re-enable centering for the focus effect
      }
    },
    // Sync with external user thumbnails if you still use them
    on: {
      slideChange: function () {
        var activeIndex = this.realIndex;
        document.querySelectorAll(".cs-user-single").forEach(function (item) {
          item.classList.remove("active");
        });
        var activeUserItem = document.querySelector('.cs-user-single[data-slide="' + activeIndex + '"]');
...
$(".reveal-img-wrap").each(function () {
  ...
  // Speed adjusted: 1.5s is usually the 'sweet spot' for smooth reveals
  const fastDuration = 1.5; 
  const fastEase = "power4.out"; 

  if ($(this).hasClass("vertical")) {
    tl.fromTo(
      inner,
      { xPercent: 110 },
      { xPercent: 0, duration: fastDuration, ease: fastEase },
      0
    );
    tl.fromTo(
      img,
      { xPercent: -50, scale: 1.2 },
      { xPercent: 0, scale: 1, duration: fastDuration, ease: fastEase },
      0
    );
